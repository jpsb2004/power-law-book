...
export function validateSnapshot(next, previous) {
  ...
  if (previous?.positions?.length) {
    ...
    const violent = moves.filter((m) => Math.abs(m.move) > 25);
    // One position can genuinely move 25% on earnings. Several at once, on a
    // book this diversified, means the data is wrong -- not the market.
    if (violent.length >= 3) {
      problems.push(
        `${violent.length} positions moved >25% since the last snapshot ` +
